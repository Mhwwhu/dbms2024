...
class IxManager {
   ...
   RC create_index(const std::string &ix_name, const std::vector<ColMeta>& index_cols) {
        ...
        // Open index file
        int fd;
        disk_manager_->open_file(ix_name, fd);
    ...
    RC open_index(const std::string &ix_name,std::unique_ptr<IxIndexHandle>& ih) {
        RC rc = RC::SUCCESS;
        int fd;
        if(RM_FAIL(rc = disk_manager_->open_file(ix_name,fd))){
            return rc;
        };
        ih = std::make_unique<IxIndexHandle>(disk_manager_, buffer_pool_manager_, fd);
        return rc;
    }
